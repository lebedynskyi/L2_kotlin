...
/**
 * This file is based on the Blowfish Engine that is part of the BouncyCastle JCE Copyright (c) 2000 The Legion Of The Bouncy Castle (http://www.bouncycastle.org)
 *
 * The only 1 reason why we need to use it is a different of ENDIANS in CLIENT and HERE..
 * It affects methods [bits32ToBytes] and [bytesTo32bits]
 */
...
class BlowfishEngine {
    ...
    fun processBlock(`in`: ByteArray, inOff: Int, out: ByteArray, outOff: Int): Int {
        checkNotNull(workingKey) { "Blowfish not initialised" }
        if (inOff + blockSize > `in`.size) throw IOException("input buffer too short")
        if (outOff + blockSize > out.size) throw IOException("output buffer too short")
        if (encrypting) encryptBlock(`in`, inOff, out, outOff) else decryptBlock(`in`, inOff, out, outOff)
        return blockSize
    }
